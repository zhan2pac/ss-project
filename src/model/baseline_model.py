...
class BaselineModel(nn.Module):
    ...
    def __init__(self, c_sources=2, length_seg=16, dim=128):
        """
        Args:
            c_sources (int): number of target sources.
            length_seg (int): length of the segment from input audio.
            dim (int): number of generated features from segment.
        """
        super().__init__()
        self.c_sources = c_sources

        self.encoder = nn.Conv1d(1, dim, kernel_size=length_seg, stride=length_seg // 2)
        self.generate_masks = nn.Conv1d(dim, dim * c_sources, kernel_size=1)
        self.mask_act = nn.Softmax(dim=0)
    ...
    def forward(self, mixture, **batch):
        ...
        masks = self.mask_act(torch.stack(masks, dim=0))  # [c_sources, batch_size, dim, time]
